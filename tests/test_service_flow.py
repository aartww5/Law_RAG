...
def test_generator_passes_runtime_ollama_limits(monkeypatch) -> None:
    llm_module = importlib.import_module("legal_rag.generation.llm")
    ...
    monkeypatch.setattr(llm_module, "importlib", fake_importlib, raising=False)
    install_fake_ollama(monkeypatch, fake_chat)
    ...
    generator = llm_module.SimpleGenerator(
    ...
    assert captured["options"]["num_ctx"] == 24576
    assert captured["options"]["num_predict"] == 1536


def test_stream_generator_logs_done_reason(monkeypatch, caplog) -> None:
    llm_module = importlib.import_module("legal_rag.generation.llm")
    ...
    def fake_chat(*, model, messages, stream, options):
        ...
        return iter(
            [
                ...
                {"message": {"content": "answer"}, "done": True, "done_reason": "length"},
            ]
        )
    ...
    generator = llm_module.SimpleGenerator(model_name="demo-model")
...
def test_decomposer_requests_json_without_thinking(monkeypatch) -> None:
    decomposition_module = importlib.import_module("legal_rag.utils.query_decomposition")
    captured: dict[str, object] = {}

    def fake_chat(*, model, messages, stream, think=None, format=None, options=None):
        captured["model"] = model
        captured["stream"] = stream
...
def test_decomposer_drops_english_variants_for_chinese_question(monkeypatch) -> None:
    decomposition_module = importlib.import_module("legal_rag.utils.query_decomposition")
    ...
    decomposer = decomposition_module.OllamaQueryDecomposer(model_name="demo-model", max_queries=4)
    variants = decomposer.decompose("小马救人造成儿童挫伤要赔偿吗")
...
def test_service_close_releases_retriever_resources() -> None:
    class Closable:
    ...
    service = LegalAssistantService(
        ...
        mini_retriever=mini,
        generator=SimpleGenerator(enable_ollama=False),
        rewriter=QueryRewriter(enable_ollama=False),
    )

    service.close()

    assert hybrid.closed is True
...
def test_service_passes_hybrid_index_config_to_retriever(monkeypatch, tmp_path: Path) -> None:
    ...
    def fake_from_articles(articles, *, index_config=None, **kwargs):
        ...
        captured["embedding_device"] = index_config.embedding_device
        captured["reranker_device"] = index_config.reranker_device
    ...
    assert captured["embedding_device"] == "cuda"
    assert captured["reranker_device"] == "cuda"
